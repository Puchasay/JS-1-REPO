...
while (continueGame) {
	...
	if (guess === numberToGuess) {
        ...
        scores+=100;
        console.log("Congrats! Your total scores now is : ", scores, "with", attempts, "attempts");
        alert(`Great success! You guessed the correct answer after ${attempts} attempt(s). Your scores is : ${scores}`);
        totalSco();
		console.log(totalScores);
		let x = Math.max.apply(null, totalScores);
		//alert(`Yuhuu!! your new highest score now is : ${x}`);
		console.log(x);
		console.log("New random number : ", numberToGuess);

        alert(`Your highest scores until now is : ${x}`);
